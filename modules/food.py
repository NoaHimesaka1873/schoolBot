...
def getfood(schoolname: str, _date: str, foodtype: int):
    ...
    try:
        ...
        schoolmeal = asyncio.new_event_loop().run_until_complete(
            neis.mealServiceDietInfo(AOSC, SSC, MLSV_YMD=int(_date), MMEAL_SC_CODE=foodtype))
...
async def main(ctx, args):
    ...
    if foodtype == 4:
        ...
        if breakfast is not None:
            ...
            embed1.add_field(name=f"{datetime.strptime(_date, '%Y%m%d').strftime('%Y년 %m월 %d일')} ({ftreference[1]})",
                             value=breakfast)
        ...
        if lunch is not None:
            ...
            embed2.add_field(name=f"{datetime.strptime(_date, '%Y%m%d').strftime('%Y년 %m월 %d일')} ({ftreference[2]})",
                             value=lunch)
        ...
        if dinner is not None:
            ...
            embed3.add_field(name=f"{datetime.strptime(_date, '%Y%m%d').strftime('%Y년 %m월 %d일')} ({ftreference[3]})",
                             value=dinner)
